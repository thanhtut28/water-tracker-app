...
const Wave = ({ size, value }: Props) => {
  const radius = size * 0.5 // outer circle
  const circleThickness = radius * 0.025 // 0.05 just coefficient can be anything you like

  const circleFillGap = 0 * radius // 0.05 just coefficient can be anything you like
  const fillCircleMargin = circleThickness + circleFillGap
  const fillCircleRadius = radius - fillCircleMargin - 2 // inner circle radius
  ...
  const waveCount = 1.2 // how many full waves will be seen in the circle
  ...
  const textWidth = 56 // get text width
  const textTranslateX = radius - textWidth * 0.3 // calculate text X position to center it horizontally
  ...
  const text = useDerivedValue(() => {
    // derived value for the Text component
    return `${((textValue.value * 8) / 100).toFixed(0)}` // convert to string
  }, [textValue])
  ...
  const clipPath: any = useDerivedValue(() => {
